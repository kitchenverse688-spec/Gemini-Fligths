        ...
        results: {
            ...
            items: {
                ...
                properties: {
                    ...
                    baggageAllowance: {
                        type: Type.OBJECT,
                        description: "Details about baggage allowance.",
                        properties: {
                            carryOn: { type: Type.STRING, description: "Carry-on baggage allowance, e.g., '1 bag, 7kg'." },
                            checked: { type: Type.STRING, description: "Checked baggage allowance, e.g., '2 bags, 23kg each'." }
                        },
                        required: ["carryOn", "checked"]
                    }
                },
                 required: ["airline", "flightNumber", "departure", "arrival", "stops", "duration", "price", "currency", "bookingUrl", "airlineLogoUrl", "baggageAllowance"]
            },
        },
        aiSuggestions: {
            type: Type.OBJECT,
...
export const fetchFlightData = async (input: FlightSearchInput): Promise<FlightData> => {
    ...
    const prompt = `
        ...
        7.  Provide realistic baggage allowance details for both carry-on and checked luggage.
    `;
        ...
        results: {
            ...
            items: {
                ...
                properties: {
                    ...
                    userReview: {
                        type: Type.OBJECT,
                        description: "User review score and count.",
                        properties: {
                            score: { type: Type.NUMBER, description: "User review score, e.g., 8.9." },
                            count: { type: Type.INTEGER, description: "Total number of user reviews, e.g., 1254." }
                        },
                        required: ["score", "count"]
                    }
                },
                required: ["name", "address", "stars", "amenities", "roomType", "pricePerNight", "totalPrice", "currency", "bookingUrl", "imageUrl", "userReview"]
            },
        },
        aiSuggestions: {
            type: Type.OBJECT,
...
export const fetchHotelData = async (input: HotelSearchInput): Promise<HotelData> => {
    ...
    const prompt = `
        ...
        7.  Generate a realistic user review score (between 7.0 and 9.8) and a plausible number of reviews.
    `;
