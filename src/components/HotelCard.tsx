...
const ReviewBadge: React.FC<{ score: number; count: number }> = ({ score, count }) => {
  ...
  const bgColor = score >= 9 ? 'bg-blue-600' : score >= 8 ? 'bg-green-600' : score >= 7 ? 'bg-yellow-500' : 'bg-slate-500';

  return (
    <div className="flex items-center gap-2">
      <div className={`text-white font-bold text-sm rounded-md px-2 py-1 ${bgColor}`}>
        {score.toFixed(1)}
      </div>
      <div className="text-sm">
        <p className="font-semibold text-slate-700">{getRatingText(score)}</p>
        <p className="text-xs text-slate-500">{count.toLocaleString()} reviews</p>
      </div>
    </div>
  );
};
      ...
      <div className="flex-grow p-4 md:p-6 flex flex-col justify-between">
        <div>
          <div className="flex justify-between items-start">
            <h3 className="text-xl font-bold text-slate-800 flex-1 pr-4">{hotel.name}</h3>
            <ReviewBadge score={hotel.userReview.score} count={hotel.userReview.count} />
          </div>
